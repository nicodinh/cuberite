...
public:
	...
	/** Base class for strategies for placing the starting pieces vertically.
	...
	public:
		...
		virtual bool InitializeFromString(const AString & a_Params) = 0;

		/** Called when the piece pool is assigned to a generator,
		so that the strategies may bind to the underlying subgenerators. */
		virtual void AssignGens(int a_Seed, cBiomeGenPtr & a_BiomeGen, cTerrainHeightGenPtr & a_TerrainHeightGen) {}
	};
	...
	class cVerticalStrategyFixed:
		public cVerticalStrategy
	{
	public:
		cVerticalStrategyFixed(void):
			m_Height(-1)  // Default to "unassigned" height
		{
		}

		cVerticalStrategyFixed(int a_Height):
			m_Height(a_Height)
		{
		}
		...
		{
			return StringToInteger(a_Params, m_Height);
		}
	...
	bool SetVerticalStrategyFromString(const AString & a_StrategyDesc);

	/** Returns true if the piece has a vertical strategy assigned to it. */
	bool HasVerticalStrategy(void) const { return (m_VerticalStrategy != nullptr); }
